...
const register = async (req, res, next) => {
  ...
  await Admin.create(admin)
    .then(async (admin) => {
      const { uniqueId, name, email } = admin.toJSON();
      console.log(uniqueId, name, email)
      // const token = generateToken({ uniqueId, name, email });
      // if (token.length !== 0) {
        await Roles
          .findOne({ where: { name: "superAdmin" } })
          .then((role) => {
            admin.setRole(role);
          });
        ...
        let activationURL = path.join(url);
        let replaceData = {
          ADMIN_NAME: name,
          NAME: "Drema",
          URL: activationURL,
        ...
        html = await getHtmlContent("welcomeAdmin_template", replaceData);
...
const forgotPassword = async (req, res, next) => {
  ...
  if (!admin) {
    ...
    // console.log(token);
    const url = process.env.ADMIN_FRONTEND_URL;
    let activationURL = path.join(url, `/v1/admin/resetPassword?secret=${token}`);
